"use strict";
/**
 @typedef {import("ace-code").Ace.Editor} Editor
...
export class SearchBox {
    ...
    $init() {
        ...
        event.addListener(sb, "click", function (e) {
            ...
            if (action
                && _this[action]) _this[action](); else if (_this.$searchBarKb.commands[action]) _this.$searchBarKb.commands[action].exec(
                _this);
            event.stopPropagation(e);
        });
    ...
    /**
     * @param {import("ace-code/src/range").Range} range
     */
     ...
     * @param {boolean} [preventScroll]
     * @param {(arg0: import("../extension").SearchResultCallbackArgs) => void} [callback]
     */
     ...
     * @param {(results?: import("../extension").SearchResultCallbackArgs) => void} [callback]
     */
    execFind(options, callback) {
        ...
        _execFind(this.editor.session, options, (result) => {
            if (result == "waiting") {
                this.counterResults = null;
